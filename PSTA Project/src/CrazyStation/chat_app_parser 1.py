    ...
    if selected_dependency == "Create documentation":
        ...
        for name in program_name:
            ...
            if found:
                ...
                for chunk_type in os.listdir(temp_output):
                    ...
                    for chunk_file in os.listdir(chunk_dir):
                        with open(os.path.join(chunk_dir, chunk_file), 'r') as f:
                            ...
                            question = "Give documentation in markdown for this code section focusing on: General Information, Change Log, inputs and outputs, Program Structure, Subprograms and Procedures, Error Handling. Translate all comments to English."
                ...
                # print(f"Summaries: {all_chunk_summaries[10:]}")
                # print(f"Merged content: {all_chunks[10:]}")
                ...
                documentation_sections = [
                    {
                        ...
                        "prompt": """Create section 1 of the documentation (1-2 pages):
                    ...
                    {
                        "title": "2. Program Architecture",
                        "prompt": """Create section 2 of the documentation (1-2 pages):
                    ...
                    },
                    {
                        ...
                        "prompt": """Create section 3 of the documentation (5-6 pages):
                           A. Procedures and Functions (2-3 pages):
                              - Document each procedure's specific purpose
                              - Detail input/output parameters for each
                              - List validation rules and error handling
                              - Explain business rules implementation
                              - Note performance considerations
                              - Map dependencies between procedures

                           B. Call Hierarchy (2 pages):
                              - Draw complete call tree with relationships
                              - Detail all execution paths
                              - List conditional calling patterns
                              - Document recursive relationships
                              - Map module dependencies"""
                    },
                    {
                        "title": "4. Data Management",
                        "prompt": """Create section 4 of the documentation (4-5 pages):
                           A. Data Structures (2 pages):
                              - Detail each structure's purpose and fields
                              - Document data types and constraints
                              - Explain validation rules
                              - Show usage patterns
                              - List relationships between structures

                           B. File Operations (1-2 pages):
                              - Document all input/output files
                              - Detail file formats and structures
                              - Explain file handling procedures
                              - List error handling for files

                           C. Data Flow (1-2 pages):
                              - Map data transformation logic
                              - Document validation processes
                              - Explain data persistence methods"""
                    },
                    {
                        "title": "5. Control Flow",
                        "prompt": """Create section 5 of the documentation (3-4 pages):
                           - Document main process flow step by step
                           - Detail all decision points and conditions
                           - Map loop structures and their purposes
                           - Explain complex conditional logic
                           - List business rule implementations"""
                    },
                    {
                        "title": "6. Error Handling",
                        "prompt": """Create section 6 of the documentation (1 pages):
                    ...
                    },
                    {
                        ...
                        "prompt": """Create section 7 of the documentation (1 pages):
                ...
                # After processing all chunks, before documentation sections
                summaries = "\n\n".join(all_chunk_summaries)

                section_contents = []
                for i, section in enumerate(documentation_sections, start=1):
                    section_prompt = ChatPromptTemplate.from_template("""
                    Program: {program_name}
                    
                    Keep in mind this is only {i}th Section from a documentation containing 7 Sections in total, 
                    which have to be fluently connected to each other.
                    
                    Based on these parsed code sections:
                    {summaries}
                    ...
                    - Include exact procedure names and parameters
                    - Document actual business rules found
                    ...
                    new_prompt = section_prompt.format(
                        ...
                        i=i,
                        summaries=summaries,
                        section_requirements=section["prompt"]
                    )
                    ...
                    # print(f"Section {i} content: {section_content}")
                    ...
                    # Remove program name only from the remaining content
                    rest_content = rest_content.replace(f"Documentation for Code Section {name}\n", "")
                    section_contents.append(f"# {section['title']}\n\n{first_line}\n{rest_content}")

                # Create TOC
                # TODO: check the TOC and formating
                toc = "## Table of Contents\n\n"
                for section in section_contents:
                    section_lines = section.split('\n')
                    if section_lines:
                        toc += f"### {section_lines[0].replace('# ', '')}\n"

                # Process first section to split title
                first_section_lines = section_contents[0].split('\n', 1)
                # TODO: remove?
                title_line = first_section_lines[0]
                remaining_content = first_section_lines[1] if len(first_section_lines) > 1 else ''

                # Combine all content with TOC between title and first section
                final_summary = f"{title_line}\n\n{toc}\n\n{remaining_content}\n\n{''.join(section_contents[1:])}"
